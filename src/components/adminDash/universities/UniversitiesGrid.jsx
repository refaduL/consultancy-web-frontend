...
export default function UniversitiesGrid({ universities, onDelete }) {
        ...
        <table className="w-full">
          ...
          <tbody>
            {universities.map((uni) => (
              ...
              >
                <td className="px-4 py-3 text-slate-900 font-medium">{uni.name}</td>
                <td className="px-4 py-3 text-slate-700">{uni.country}</td>
                <td className="px-4 py-3 text-center text-slate-700">{uni.programs.length}</td>
                <td className="px-4 py-3 text-center">
                  <span className="px-2 py-1 text-xs bg-blue-100 uppercase">{uni.type}</span>
                </td>
                <td className="px-4 py-3 text-center">
                  <div className="flex items-center gap-2 justify-center">
                    <button
                      onClick={() => navigate(`/admindashboard/universities/${uni._id}?mode=view`)}
                      className="p-1.5 bg-slate-50 hover:bg-slate-100 rounded-lg"
                    >
                      <Eye className="w-4 h-4 text-slate-700" />
                    </button>
                    <button
                      onClick={() => navigate(`/admindashboard/universities/${uni._id}?mode=edit`)}

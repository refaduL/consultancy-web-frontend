    ...
    >
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-4">
          ...
          >
            {user.application?.status === "approved" && <CheckCircle className="w-7 h-7 text-green-600" />}
            {user.application?.status === "rejected" && <XCircle className="w-7 h-7 text-red-600" />}
            {user.application?.status === "pending_review" && <Clock className="w-7 h-7 text-yellow-600" />}
          </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-3 flex-wrap">
              <h2 className="text-lg sm:text-2xl md:text-3xl font-bold text-slate-900 truncate">
                {user.application?.status === "approved" && "Congratulations! You're Approved"}
                {user.application?.status === "rejected" && "Application Rejected"}
                {user.application?.status === "pending_review" && "Application Under Review"}
              </h2>
              <span
                className={`px-3 py-1 rounded-full text-sm font-semibold w-fit whitespace-nowrap
                  ${user.application?.status === "approved" ? "bg-green-600 text-white" : ""}
                  ${user.application?.status === "rejected" ? "bg-red-600 text-white" : ""}
                  ${user.application?.status === "pending_review" ? "bg-yellow-500 text-white" : ""}`}
              >
                {user.application?.status.replace("_", " ").toUpperCase()}
              </span>
            </div>
            ...
            {user.application?.status !== "pending_review" && (
